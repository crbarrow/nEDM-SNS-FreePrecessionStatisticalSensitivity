...
tau_walls = 1; %%%%---- SEE LOOPS FOR VALUE ----
...
tau_beta = 885;
tau_buildup = (1/tau_beta+1./tau_walls).^-1; %when accumulating, UCNs are aligned with 3He. ---- note tau_walls dependence
...
f3n = omega3n/(2*pi);
phi0 = 1; % phase angle of UCN precession %%%%%---- SEE LOOP FOR VALUE ----

t_width = 20E-3; %[s]

numLoops = 1000;
doneRes = 100/numLoops;

diff_f_LS_ray = zeros([1 numLoops]);
diff_f_RLS_ray = zeros([1 numLoops]);
diff_f_ML = zeros([1 numLoops]);

LStime = zeros([1 numLoops]);
RLStime = zeros([1 numLoops]);
MLEtime = zeros([1 numLoops]);

sigma_f_RLS = zeros([1 numLoops]);

D_f_LS = zeros([1 numLoops]);
D_f_RLS = zeros([1 numLoops]);
D_f_MLE = zeros([1 numLoops]);
    ...
    for j = 1:2
        ...
        for k = 1:numLoops
            ...
            %---- Recursive Least Squares fitting to single exponential
                ...
                sigma_f_RLS(k) = sigma_f;
                diff_f_RLS_ray(k) = diff_f_RLS;
...
%            
            ...
            D_f_LS(k) = diff_f_LS_ray(k);  % populates array of frequency difference (Least Squares fitting)
            D_f_RLS(k) = diff_f_RLS_ray(k);  % populates array of frequency difference (Recursive Least Squares)
            D_f_MLE(k) = diff_f_ML(k);  % populates array of frequency difference (MLE with Poisson distribution)
            progress = k*doneRes/4 + 50*(i-1) + 25*(j-1)
        end
